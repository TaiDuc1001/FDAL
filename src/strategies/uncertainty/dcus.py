...
class DCUSStrategy(BaseStrategy):
    ...
    def _ensure_classwise_quality(self):
        ...
        if self.experiment_dir:
            alfi_quality_path = Path(self.experiment_dir) / f"round_{self.round}" / "alfi_classwise_quality.npy"
            ...
            if alfi_quality_path.exists():
                classwise_quality = np.load(alfi_quality_path)
                ...
                logger.warning(f"Neither ALFI quality file ({alfi_quality_path}), "
